...
Russian language more long than English. More word-ending. More filler word. More polite wrapper. Caveman fix.

Russian Caveman cut filler intros, polite wrappers, parasitic constructions. Arrow and colon replace long connectives. Code, URL, path, API, JSON — untouched.
...
|-------|---------|-----------|---------|
| **ru-lite** | `/caveman ru-lite` | Soft. Full sentences. Minimal filler. | user-facing text, docs |
| **ru-full** | `/caveman ru-full` (or `/caveman ru`) | Short phrases. Drop pronouns. Fragments OK. | tech explanations, code review, debug |
| **ru-ultra** | `/caveman ru-ultra` | Telegraphic. Arrows, colons, markers. | agent answers, summaries |
| **ru-notes** | `/caveman ru-notes` | Shorthand notes. Maximum compression. | notes for self / another agent |
...
Three ways to tell caveman which language to start in:

**1. Per-turn command** (works anywhere):
```
/caveman ru           # ru-full (default Russian level)
/caveman ru-lite
/caveman ru-full
/caveman ru-ultra
/caveman ru-notes
/caveman-ru ultra     # equivalent alternative command
/caveman full         # back to English
```

**2. Default language via env** (Claude Code `env` in settings.json):
```json
{
  "env": {
    "CAVEMAN_LANG": "ru"
...
```
When `CAVEMAN_LANG=ru`, session start in `ru-full` instead of `full`, and hook inject Russian rules as SessionStart context. Set `"en"` (or omit) for default English.

Also work as shell export: `export CAVEMAN_LANG=ru`

**3. Auto-detect by prompt language** (opt-in):
```json
{
  "env": {
...
```
With auto-detect on, `UserPromptSubmit` hook look at each prompt. If ≥30% of letters Cyrillic (code blocks stripped first), caveman flip `full`/`lite`/`ultra` → `ru-full`/`ru-lite`/`ru-ultra` automatically. Mostly-Latin prompt (≤5% Cyrillic) flip back. Explicit `/caveman` commands always win over auto-detect.
...
- **ru-lite** — writing to users or docs. Clarity + politeness matter.
- **ru-full** — tech explanations to developers. Default for most cases.
- **ru-ultra** — agent-to-agent, summaries, context already known.
- **ru-notes** — note "for future self". Mix RU + EN tech terms freely.

#### When NOT to use ultra / notes

- security warnings;
- irreversible operations (`DROP`, `rm -rf`, force push);
- step-by-step instructions where wrong order break things;
- user confused or asking to repeat.

Auto-Clarity: mode auto-exit compression for these cases, resume after.
