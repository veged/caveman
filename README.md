...
**2. Default language in `~/.claude/settings.json`** (Claude Code SessionStart hook reads this on every session start):
```json
{
  "caveman": {
    "lang": "ru"
...
```
When `lang: "ru"`, the session starts in `ru-full` instead of `full`, and the hook injects Russian rules as SessionStart context. Set `"lang": "en"` (or omit the field) for default English behavior.

**Or via env var** (overrides settings.json):
```bash
...
```
With auto-detect on, the `UserPromptSubmit` hook looks at each prompt. If ≥30% of its letters are Cyrillic (code blocks stripped out first), caveman flips `full`/`lite`/`ultra` → `ru-full`/`ru-lite`/`ru-ultra` automatically. A mostly-Latin prompt (≤5% Cyrillic) flips back. Explicit `/caveman` commands always win over auto-detect.

Env equivalent: `CAVEMAN_AUTO_DETECT_LANG=1`.

**Statusline** shows the active mode, e.g. `[CAVEMAN:RU-FULL]`, so you can see at a glance which language + level is active.

**Other agents** (Cursor, Windsurf, Cline): the sync workflow ships Russian rule files alongside the English ones — `.cursor/rules/caveman-ru.mdc`, `.windsurf/rules/caveman-ru.md`, `.clinerules/caveman-ru.md`. Cursor/Windsurf Russian rules are **opt-in** (agent-requested / model-decision triggers), so the English rule stays the default. Cline loads both. Copilot keeps English-only to avoid mixing repo-wide instructions; Copilot users who want Russian should set `caveman.lang: ru` in their local `~/.claude/settings.json` or reference `skills/caveman-ru/SKILL.md` directly.
